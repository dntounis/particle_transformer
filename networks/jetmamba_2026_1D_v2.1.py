...
class AnchorAttentionBlock(nn.Module):
    def __init__(self, d_model, num_heads=8, num_anchors=8, dropout=0.1, ffn_expand=4):
        super().__init__()
    ...
    def forward(self, x, valid_mask):
        ...
        anchors = x_norm[:, :num_anchors, :]
        anchor_mask = valid_mask[:, :num_anchors]

        # Ensure each batch item has at least one unmasked anchor to avoid NaNs.
        safe_anchor_mask = anchor_mask.clone()
        no_valid_anchors = ~safe_anchor_mask.any(dim=1)
        if num_anchors > 0:
            safe_anchor_mask[no_valid_anchors, 0] = True

        attn_out, _ = self.attn(
            x_norm,
            anchors,
            anchors,
            key_padding_mask=~safe_anchor_mask,
            need_weights=False,
        )
...
class JetMamba2026_1D_v2_1(nn.Module):
    def __init__(
        self,
        num_classes=10,
        d_model=128,
        ...
        **kwargs
        ...
        super().__init__()
        if Mamba is None:
            raise ImportError("mamba_ssm is required for jetmamba_2026_1D_v2.1") from _MAMBA_IMPORT_ERROR
        ...
        if attention_every is None or attention_every <= 0:
            attention_indices = []
        else:
            attention_indices = list(range(attention_every - 1, max(n_layers - 1, 0), attention_every))
        self.attention_indices = attention_indices
        self.attention_layers = nn.ModuleDict(
            {
                str(layer_idx): AnchorAttentionBlock(
                    d_model=d_model,
                    num_heads=num_heads,
    ...
    def forward(self, *args, **kwargs):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if layer_key in self.attention_layers:
                x = self.attention_layers[layer_key](x, valid_mask)
...
def get_model(data_config, **kwargs):
    ...
    model = JetMamba2026_1D_v2_1(**cfg)
    model_info = {
